...
熊旗形态特征:
1. 前期有明显的下跌趋势（旗杆）
2. 随后出现小幅反弹整理（旗面），通常呈上升通道或三角形
3. 整理期间成交量逐渐萎缩
4. 价格跌破旗面下轨时成交量放大，确认突破
...
def detect_bear_flag(df: pd.DataFrame, min_pole_height_pct: float = 0.08,
    ...
    返回:
        List[Dict]: 检测到的熊旗形态列表，每个字典包含形态详情
    """
    ...
    for pole_bars in range(min_pole_bars, max_pole_bars + 1):
        ...
        for pole_end in range(pole_bars, len(df) - min_flag_bars):
            # 检查前面的pole_bars根K线是否构成旗杆（强势下跌）
            pole_section = df.iloc[pole_end-pole_bars:pole_end+1]
            
            # 旗杆条件：价格明显下跌
            pole_start_price = pole_section['high'].iloc[0]
            pole_end_price = pole_section['low'].iloc[-1]
            pole_height_pct = (pole_start_price - pole_end_price) / pole_start_price
            
            if pole_height_pct < min_pole_height_pct:
                continue  # 旗杆高度不足
            
            # 检查是否有至少60%的K线是下跌的
            min_down_candles = int(len(pole_section) * 0.6)
            down_candles = sum(1 for i in range(len(pole_section)) 
                              if pole_section['close'].iloc[i] < pole_section['open'].iloc[i])
            if down_candles < min_down_candles:
                continue  # 下跌K线不足
            ...
            for flag_length in range(min_flag_bars, min(max_flag_bars + 1, len(df) - pole_end)):
                ...
                # 旗面上涨幅度不应过大（相对于旗杆高度）
                flag_rise_pct = (flag_end_close - flag_start_close) / flag_start_close
                if flag_rise_pct > pole_height_pct * 0.5:  # 旗面反弹不应超过旗杆高度的50%
                    continue
                ...
                # 旗面应该是上升趋势或横盘
                if flag_slope <= 0:  # 必须是上升趋势
                    continue
                    
                # 旗面的高度应该在合理范围内（相对于旗杆高度）
                flag_height_ratio = flag_range / (pole_start_price - pole_end_price)
                ...
                if pole_end + flag_length < len(df):
                    ...
                    if (breakout_candle['close'] < flag_low and 
                ...
                # 计算形态强度
                strength = _calculate_bear_flag_strength(
...
def _calculate_bear_flag_strength(pole_height_pct: float, flag_length: int, 
...
def is_bear_flag_breakout(df: pd.DataFrame, flag_pattern: Dict) -> bool:
    """
    检查熊旗形态是否已经突破
    ...
    # 突破条件：收盘价低于旗面低点且成交量放大
    flag_avg_volume = flag_section['volume'].mean()
    return (breakout_candle['close'] < flag_low and 
            breakout_candle['volume'] > flag_avg_volume * 1.5)
